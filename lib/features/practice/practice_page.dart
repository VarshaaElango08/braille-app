...
class _PracticePageState extends State<PracticePage> {
  ...
  bool _isAutoAnswerFlowRunning = false;
  bool _answerHandledForCurrentQuestion = false;
  ...
  Future<void> _cancelAllActiveAudioFlows() async {
    _cancelAutoFlowTimers();
    _isAutoAnswerFlowRunning = false;
    await _stopMicIfNeeded();
  }
  ...
  void _makeQuestion() {
    ...
    options = o;
    correctIndex = o.indexWhere((e) => e.id == it.id);
    _answerHandledForCurrentQuestion = false;
  ...
  Future<void> _goHome() async {
    final speechRate = await AppSettings.getSpeechRate();
    await _cancelAllActiveAudioFlows();
    await ProgressService.markModuleOpened("Home");
    await tts.speak(
      _narrate("Going to Home page.", "ஹோம் பக்கத்திற்குச் செல்கிறேன்."),
      rate: speechRate,
  ...
  Future<void> _speakQuestion({bool intro = false}) async {
    ...
    await tts.speakBlocking(msg, rate: speechRate);
    _scheduleAutomaticAnswerFlow();
  }

  void _scheduleAutomaticAnswerFlow() {
    _cancelAutoFlowTimers();

    _autoPromptTimer = Timer(const Duration(seconds: 10), () async {
      if (!mounted || _answerHandledForCurrentQuestion || _movingToNextQuestion) {
        return;
      }
      await _startAutomaticAnswerListening();
    });
  }

  Future<void> _startAutomaticAnswerListening() async {
    if (!mounted || _answerHandledForCurrentQuestion || _movingToNextQuestion) {
      return;
    }
    ...
    _isAutoAnswerFlowRunning = true;

    await tts.speakBlocking(
      _narrate(
        "Now tell your answer.",
        "இப்போது உங்கள் பதிலை சொல்லுங்கள்.",
      ),
      rate: speechRate,
    );
    ...
    await voice.startListening(onResult: (words) async {
      if (!mounted || _answerHandledForCurrentQuestion) return;
    ...
    _autoStopTimer = Timer(const Duration(seconds: 18), () async {
      if (!mounted || _answerHandledForCurrentQuestion) return;
  ...
  Future<void> _handleNoValidAnswer() async {
    if (_answerHandledForCurrentQuestion || !mounted) return;

    _answerHandledForCurrentQuestion = true;
    _isAutoAnswerFlowRunning = false;
    ...
    await tts.speakBlocking(
      _narrate(
        "Time is over. No valid answer received. Correct answer is ${o[correctIndex].name}.",
        "நேரம் முடிந்தது. சரியான பதில் கிடைக்கவில்லை. சரியான பதில் ${o[correctIndex].name}.",
      ),
      rate: speechRate,
    );
  ...
  void _next() {
    if (_movingToNextQuestion) return;
    _movingToNextQuestion = true;
    _cancelAllActiveAudioFlows();
